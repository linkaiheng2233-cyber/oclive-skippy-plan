...
| GS-ARCH-001 | **P1** | OPEN · blocks 单节点形态（**2026-09-23 静态核验**；待形态裁决） | **感知内核在结构上无法表达「一块板承担全部感知」**（ADR-063 / `ESP32_GUN_ASSISTANT.md` 的单 ESP32 形态会直接撞上）：① `PerceptionProfile::try_new`（`reducer/profile.rs:113`）**强制两个不同** `NodeUid`，相同即 `ProfileError::DuplicateNodeBinding`；② `NodeRole` 是 `reducer.rs:51` 的**私有两值枚举**，`node_role()` 按 UID 硬分派；③ `(role, observation)` 绑定表是**白名单**（`reducer.rs:495–522`），越界组合返回 `CapabilityNotBound`——因此一块节点**不能同时**上报前握接触 + 后握接触 + 姿态估计 + 扳机触点。附带缺口：**表达侧（下行）类型一个都还没有**——`AssetRef`、表达指令、模式（自由/预设）、能力协商/hello 均只在文档（`64A-R1`）而未进 `contracts` Rust 真源 | 把「两个固定角色」泛化为**「节点 → 能力集映射」**，reducer 按能力分派并同时落地能力协商（推荐）；不采用「单节点冒充两个 `node_uid`」（伪造拓扑、破坏诊断）。**不需要硬件**，可在桌面用模拟器完成；关闭条件：新 ADR + `contracts` 增补能力协商与表达侧类型 + reducer 泛化 + 回放/降级测试与 fixtures 同步，并跑完整 `./scripts/verify.ps1` |
