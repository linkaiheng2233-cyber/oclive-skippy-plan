...
状态：CANDIDATE（2026-09-23；项目所有者提出「结合电脑做空枪打靶、投影靶场，并参考游戏做靶场交互与 wargame 特色」，尚未裁决）
...
2. **契约 v0.3 的范围**：v0.2 的 `SensorObservationKind` 是**闭集枚举 + `deny_unknown_fields`**（`contracts/src/lib.rs:179`），**角速率无法"悄悄加"**，必须走新契约版本。建议把**① 能力协商（hello + 能力集）、② 角速率观测、③ 只读换弹**合并为**一次 v0.3 升级**，能力协商做载体，后两者为首批能力。**IMU 实测**（静止噪声、零偏重复性、温漂、预热时间、100 Hz 节拍抖动）与接口选型（SPI + DRDY 优先）并入 ADR-063 的台架实验清单。**相机降为按需**：仅在其启用时才需裁决它与 P1「娱乐观察」是同一条通道还是两条。
