...
状态：**ACCEPTED**（2026-09-23；项目所有者裁决：**P0-A 成为 P0 主线形态**，P0-B 保留为后续 field 形态。原为同日提出的新思路）
...
### ADR-063 补充二：运动强度决定交互密度——两种形态是互补而非取舍（2026-09-23）

所有者指出：PC 形态的代价只是「活动范围减小」，**但激烈运动时本来就不需要角色交互**；真正需要丰富体验的是**坐在电脑前、或休息时（用手机或电脑）**。据此确立一条与「距离」并列的第一变量：

> **交互密度与运动强度反相关。** 移动中只需要最低限度的陪伴；静止时才需要丰富表达。因此 PC/手机形态**不是放弃了移动场景，而是放弃了「移动场景中的交互」——而那正是本来就不需要的部分**，代价接近零。

| 场景 | 运动强度 | 宿主 | 模式 | 交互密度 |
|---|---|---|---|---|
| 下场 / 移动 / wargame 交锋 | 高 | **无** | **预设模式**（T0） | **地板**：少量提示、不打断 |
| 桌边坐下 | 低 | **PC** | 自由模式 + 娱乐后端（ADR-066） | **上限由主机性能决定** |
| 休息 / 间隙 / 沙发 | 低 | **手机**（后续立项） | 轻交互 | 中 |
| 断连 | — | — | 预设降级 | 地板 |

**三条由此推出的设计纪律**：

1. **带宽需求与链路质量同向。** 移动时需要高带宽吗？不需要；静止时链路最好，而恰在此时需要高带宽（读长文本、瞄准流、更丰富的表现）。设计因此**不跟物理对抗**——不存在「必须在运动中获得高带宽」的需求。
2. **预设模式需要「战术静默」子档。** 下场时**声音与亮光会暴露位置**，故预设模式必须能在不发声、低亮度、仅图标的状态下工作。这也回答了「移动中要不要角色说话」：默认不说。
3. **宿主的 Wi-Fi 定义包含「手机热点」。** 枪**始终作为 STA**，宿主可以是①路由器后的 PC，或②**手机热点**后的手机——后者是野外/休息区唯一现实的宿主路径，且仍是同一条代码路径，**不需要 BLE 兜底**（维持形态级决策 D2）。把 OCLive 宿主移植到手机是**独立立项**（Rust 内核可复用，UI 与生命周期需重做；手机跑 1–3B 级量化模型在当前旗舰机上可行，`ESTIMATED`、未实测），此处只登记为**宿主分层**，不排期。

**可及性收益（本补充的主要价值）**：枪侧电子约 **130–400 元** + 已有 PC 即可进入，取代原方案 1,000–2,600 元的全套门槛；**成本低 → 更多人能接受**，而体验上限仍随主机升级。
...
**形态细化（2026-09-23）**：枪侧的具体设计——**一块 ESP32-S3 + 小屏、双模式（自由/预设）、下行只传 AssetRef 不传像素、预设触发范围与契约现状、BOM 与重量预算**——见 `ESP32_GUN_ASSISTANT.md`（同为 `CANDIDATE`）。项目所有者已确认该形态意图：**枪上不再有主机；接上电脑主机即获得完整体验，体验上限由主机性能决定**。

**该形态的四项形态级决策已由所有者拍板（2026-09-23，记录于 `ESP32_GUN_ASSISTANT.md` §9.1）**：① 第一阶段走 **USB-CDC 串口 + 电脑供电**；② 无线只考虑**与电脑的连接**、正式运行用 **Wi-Fi STA**，BLE 不排期，手机作宿主后续再议；③ 枪上小屏**已定固定于导轨**，本形态不再决策安装位置；④ **先只做屏幕并预留 I2S 焊盘**，不触碰 P0「不实现扬声器」红线。阶段路径（A 桌面 USB → B 无线 → C 装枪）见该文 §9.2。
...
**⓪ 补充：一个待验证的免费实验（2026-09-23 提出，未实测，不改变上述决定顺序）**：内核命令行的连接器语法支持一个 `D` 标志（`video=HDMI-A-1:480x800@60D`），由通用 DRM 解析为 `DRM_FORCE_ON_DIGITAL`，含义是**按数字 DVI 输出处理、跳过 HDMI 的 AVI/VSIF 信息帧**。由于板端像素时序与 PC 完全一致（CRTC `480x800: 60 34860 …700…830` 即 EDID 首选 DTD），"PC 能出图、板子不能"的差异只剩**信息帧**与 **TMDS 信号裕量**两处，而该标志只去掉前者，故可把两条根因**二选一**。它与既有结论不冲突：`video=` 不做的是"覆盖 EDID 首选模式"（模式选择轴），`D` 影响的是信令方式轴。**前提**：本厂商驱动是否采用该标志尚未验证——必须比对 dmesg 的 `select vic 0 use hdmi14 vsif` 行有无变化；若不变，则本次结果无效，不能据此否定信息帧假设。做法与判读见 `handoff/AI_SESSION_HANDOFF_2026-09-23.md` §2.6。
...
**2026-09-23 更新（关键路径属性变化，决定本身不变）**：随 ADR-063 升为 `ACCEPTED`，**P0-A 主线形态不依赖板端 HDMI/DP 输出**（枪侧用 SPI 小屏，主机用 PC 显示器），因此本轮显示阻塞**已移出关键路径**，不再阻塞近期进度。上文三项决定**仍然成立且不撤销**：② 的 `USB-C DP Alt → 主动式 DP→HDMI` 验证仍应按原计划做（它是 P0-B field 形态的既定路线），③ 的 SPI 路线则由 P0-A 直接采用。只是它们从"必须立刻解决"变为"按 P0-B 的节奏推进"。

## ADR-065：感知内核的节点模型泛化为「节点 → 能力集映射」（方向冻结，落地待 ADR-063 裁决）

状态：ACCEPTED（2026-09-23：方向与约束已冻结；**ADR-063 形态已裁决，本项落地前置已解除**——代码仍未动，`GS-ARCH-001` 保持 `OPEN` 直至重构完成并跑过完整门禁）
...
**落地待办（ADR-063 形态已裁决，2026-09-23 起可执行；`GS-ARCH-001` 解锁）**：
...
**2026-09-23 更新**：ADR-063 已升为 `ACCEPTED`（P0-A 成为主线形态），**本项前置解除，重构可开工**；`GS-ARCH-001` 维持 `OPEN` 至代码落地并跑过完整门禁。
...
## ADR-066：主机侧娱乐后端（屏幕打靶 / 投影靶场）独立立项，瞄准由主机视觉闭合

状态：**ACCEPTED**（2026-09-23；随 ADR-063 形态一并裁决。原为所有者当日提出的「结合电脑做空枪打靶、投影靶场，并参考游戏做靶场交互与 wargame 特色」）

**决定一：立项与归属。** 「屏幕打靶 / 投影靶场 / 计分关卡」作为**独立方向立项**，与 OCLive **并列**消费 `DeviceEvent`，**不进入感知内核与器灵核心**（沿用 `ROADMAP.md` 既有裁决：相机、录像、AI 视觉、逐颗弹药计数必须独立立项）。游戏后端与 OCLive 互不写入：游戏数值不直接变成角色台词，角色表达仍只由 OCLive 出。

**决定二：瞄准通道（同日修订）。** 首选 **陀螺速率控制（手柄陀螺式相对瞄准）**：枪上 IMU 上报**滤波、去零偏后的角速率**，主机积分成准星位移，**不做绝对屏面映射**——因此**不需要相机、不需要标记、不需要站位与屏幕位姿标定，额外硬件 0 元**。代价是漂移与回中，由**「放下枪自动回中 + 准星边缘夹持 + 静止 ZUPT 零偏修正 + IMU 预热」**四条**结构性**措施控制（残差 0.5 °/s 即 60 s 累积约 30°，故回中是必需项而非优化项）。相机方案（主机摄像头 + 枪上刚性标记 → PnP 绝对解算）**降为按需的进阶选项**，只在需要绝对参考、小靶精度或免回中时启用，届时单独走边界门 5 评审。阶梯为：**无瞄准的节奏玩法（0 元）→ 陀螺速率瞄准（0 元）→ 相机绝对瞄准（0–40 元，按需）**。

**修订理由（2026-09-23 晚，依所有者提出的「手柄陀螺校准 → 电脑模拟 → 省掉摄像头」）**：原方案把 IMU 路径理解为「把指向映射到屏面」（**绝对几何**），于是需要站位距离与屏幕位姿标定，方位角又受磁干扰与积分漂移影响，故被判为降级。改为**速率控制**后，绝对几何这一环整体消失，IMU 立即成为 **0 元首选**：玩家的眼睛就是闭环，这正是主机手柄陀螺瞄准成立的原因。附带收益是**移除了视觉瞄准通道**——不触发边界门 5、不引入相机隐私面。

**决定三：唯一允许的几何规则。** **「瞄准射线 × 屏幕平面」求交是允许的；任何引入弹道、初速、飞行时间、下坠、风偏、命中概率或自动修正的计算都是禁止的。** 界面称**准星 / 光点**，不称「弹着点」；**禁止任何形式的辅助瞄准、吸附、命中修正、后坐补偿**——可以**测量**玩家瞄向哪里，**不可以修正**它。将来若用相机做周期性重定位，**只允许修正传感器漂移，不允许修正玩家意图**。

**三项配套约束**：

1. **延迟归属（按路径分开）**：**陀螺速率**路径的闭环**在枪 → PC 链路上**（约 100 Hz 瞄准流，约 1.2 KB/s），因此**链路 p95 与抖动**成为关键指标，**有线 USB-CDC 是默认前提**；**相机**路径的闭环在主机视觉内闭合（约 50–60 ms motion-to-photon @30 fps，`ESTIMATED`），链路只承载离散事件。两条路径下 ADR-063 的「事件到画面 p95 ≤ 100 ms」门都**不约束准星本身**，但继续约束**角色反馈**与**离散事件**。
2. **枪侧零改动**：模式位由双态扩为**三态**（自由聊天 / 靶场游戏 / 离线预设）；靶场模式的「得分 / 弹药 / 命中提示」只是**状态位扩展**。枪侧屏只显示枪娘表情 + 极简状态位，**不作为游戏主画面**。
3. **安全与文案红线**：进入靶场模式前必须完成**空仓 / 前方无人 / 不在实弹场所**三重确认（做成角色的仪式化行为）；载体仅限玩具或模型载体、只在封闭空间使用；**对外定位为娱乐性打靶游戏，不使用「训练」作为产品定位**；相机画面**只用于几何解算**，不存储、不上传、不送模型、不进角色记忆。完整清单见 `PC_SIMULATION_BACKENDS.md` §7。

**为什么仍需本 ADR（边界变更门）**：触发 `PROJECT_BOUNDARIES.md` §7 的**门 1**（新组件同时认识感知内核与 OCLive 内部对象）与**门 5**（新视觉能力涉及瞄准）；但**不触发门 2**——三种扳机检测方案均为**外部、只读、非侵入**，不进入发射器内部、不连接原火控 / 电机 / 供弹。§4 表格现有「视觉」行的禁止列已含「射击解算」，本 ADR **不修改该行**，而是新增「**游戏/模拟**」行，把「射线求交」与「射击解算」明确区分开。

**裁决前的待办（未裁决前不做）**：

1. **G0 最小闭环**：扣扳机 → PC 画面反馈，用来回答「枪当游戏输入设备好不好玩」；其**阻塞项是扳机检测的机械方案三选一**（外部微动 / 外部霍尔 + 磁铁 / 不碰枪的握把按键）。
2. **契约 v0.3 的范围**：v0.2 的 `SensorObservationKind` 是**闭集枚举 + `deny_unknown_fields`**（`contracts/src/lib.rs:179`），**角速率无法"悄悄加"**，必须走新契约版本。**版本/能力协商无需重新设计**——`NodeHello` 报 `supported_contract_versions` + capabilities、Host 取最高共同版本的设计**早已冻结**（`78A-R1` 等），缺的是实现。故 v0.3 = **实现既有 NodeHello 协商** + 抬版本，并把 **① 角速率观测与 `Capability` 成员、② `AssetRef` ID 空间、③ 只读换弹** 作为首批能力一次做完。**IMU 实测**（静止噪声、零偏重复性、温漂、预热时间、100 Hz 节拍抖动）与接口选型（**D6 已定 ICM-42688-P / SPI，QMI8658 回退**）并入 ADR-063 的台架实验清单。**相机降为按需**：仅在其启用时才需裁决它与 P1「娱乐观察」是同一条通道还是两条。
3. **与形态联动**：本方向只在 PC 主机形态（ADR-063）下成立，随该形态一并裁决。
4. **接受前不得**修改 `PROJECT_BOUNDARIES.md`、不得新增采购行、不得改动 `ROADMAP.md`；娱乐后端**先作为 `ailive-gun-spirit-host` 内的独立模块**，不急于新增第 4 个 crate（workspace 成员数有门禁断言，见 `GS-BOUNDARY-001`）。

原因：「枪娘陪你打靶」把 PC 主机形态的价值从「更大的模型」扩展到「屏幕/投影上的娱乐内容」，而它恰好只需要主机侧算力与一条已有的离散事件链路就能成立——枪上硬件零改动，却能显著提升把玩价值。同时它踩在两条红线附近（视觉瞄准、射击解算），因此必须先把**允许与禁止的分界线写成一句话规则**并入档，再谈实现；否则「打靶游戏」会顺着最省事的实现路径滑向弹道模拟与辅助瞄准，那正是本项目明令禁止的方向。
