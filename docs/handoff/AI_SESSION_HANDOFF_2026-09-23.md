...
- **当前基线**：`GS-P0-BL-2026-09-23` —— **主线形态已切换**（前一基线 `GS-P0-BL-2026-09-22`）。
- **形态（2026-09-23 裁决，`ACCEPTED`）**：**P0-A「PC 宿主 + 枪侧瘦客户端」成为 P0 主线**——枪上只留**一块 ESP32-S3（N16R8）+ SPI 小屏 + 外挂 IMU + 外部只读扳机霍尔**，小屏按既有设计固定在导轨，**PC 为唯一 Host**；原三轴一体主机舱（P0-B）保留但后置，**其实测门不取消**。形态级决策 D1–D6 与阶段路径见 `../ESP32_GUN_ASSISTANT.md` §9.1/§9.2。
- **同时新立 ADR-066**：主机侧娱乐后端（屏幕打靶 / 投影靶场），**G0 = 扣扳机 → 主机画面反馈**（无瞄准、无相机）。瞄准通道首选**陀螺速率控制**（手柄陀螺式相对瞄准，0 元、无需相机）。红线（不弹道、不辅助瞄准、对外定位娱乐而非训练）见 `../PC_SIMULATION_BACKENDS.md` §7。
- **代码/文档**：三 crate Rust workspace + v0.2 契约与生成 Schema；**本周期内没有任何代码改动**，门禁 `./scripts/verify.ps1` PASS（见 `../PROJECT_BASELINE.md` 的门禁证据）。**两个已解锁但未开工的前置**：`GS-ARCH-001`（感知内核节点模型泛化，**不需要硬件**）、`GS-CONTRACT-004`（v0.3：实现既有 `NodeHello` 协商 + 角速率观测 + `AssetRef` ID 空间 + 只读换弹）。
- **硬件（P0-B，仍未解决但已不阻塞主线）**：Zero 3W 首轮 bring-up 系统/网络/远程通过；视频链四项判据通过（`GS-HW-002` 关闭）；**但当前屏在板端不出图**（屏侧 HDMI 兼容性，`GS-HW-005`）——已购 **USB-C DP→HDMI 主动转接头（山泽）**待到货验证，**不更换主板**。**注意：P0-A 不使用板端 HDMI/DP，这条不在主线上**。
- **立刻要做的第一件事**：👉 ① **P0-A 第零批采购（约 55–180 元，含屏）**：ESP32-S3 **N16R8**、**ICM-42688-P（SPI）**（QMI8658 回退）、数字霍尔（A3144 类）+ 3×2 mm 钕磁铁、SPI 小屏（**尺寸按导轨支架实际开口**）；② 桌面 USB 阶段逐项跑通「屏幕出图 → 举枪/放下 → 扳机触点」，每步只加一个变量；③ 与硬件并行做 `GS-ARCH-001`（纯桌面、不需硬件）。P0-B 的 DP 转接头验证、横向显示落地与板卡身份采集**继续做，但不再阻塞主线**。
- **四条硬约束**：① 唯一权威是本仓，桌面/导出件只读；② 分阶段提交 + 提交前跑门禁；③ 不实现火控/瞄准/武器控制类功能；④ **扳机检测只允许外部只读耦合**（D5：磁铁贴扳机背面 + 护圈外侧霍尔，不开壳不焊接）；「通过火控读取扳机」**已评估并否决**，不要再提。
- **⏸ 挂起待答（2026-09-23 会话结束时留下，等项目所有者回答）**：
  1. **导轨支架是按几寸屏做的？** —— 这决定 SPI 小屏的下单尺寸（ST7789 类，240×320 级）。此前 D3 已定「小屏固定于导轨、沿用既有支架设计」，因此**支架尺寸就是屏尺寸的唯一依据**；这个数一出来，第零批即可整体下单。
  2. **`个人文档` 的 6 行导出映射怎么处置？** —— 该目录已被重组为索引结构（根部只剩 `README.md` + 子目录），`scripts/export-map.tsv` 里 6 个 Personal 目标在根下已不存在。**改指向实际子目录（如 `个人文档\四季宝器灵\`），还是删除这 6 行？** 详见 `TECHNICAL_DEBT.md` 的 `GS-DOC-002`。本次导出已用 `-PersonalDocs <暂存目录>` 规避，未污染重组后的目录。
  3. **是否现在开工 `GS-ARCH-001`（感知内核节点模型泛化）？** —— ADR-065 已冻结方向与三条硬约束，**形态裁决前置已解除**，约 1–2 天，**不需要任何硬件**，可在模拟器上验证。它是单 ESP32 形态真正的开工前置：不做，固件写出来也进不了内核（现在内核强制两个不同 `NodeUid`）。
...
预期：工作树干净；存在基线 tag `baseline/GS-P0-BL-2026-09-18` / `-09-22` / `-09-23`；HEAD 至少为 `34fde80`（若更新，先读新提交的 message）。
...
| 3 | `../ESP32_GUN_ASSISTANT.md` | **当前主线形态**：枪侧 ESP32 设计、决策 D1–D6、阶段路径、契约 v0.3 范围 |
| 4 | `../PC_SIMULATION_BACKENDS.md` | 主机侧娱乐后端（打靶/靶场）、瞄准通道选型与红线 |
| 5 | `../test-worksheets/runs/2026-09-23-zero3w-panel-no-image.md` | **最新**实机记录：链路已通但屏不出图的现场证据（`GS-HW-005`，属 P0-B） |
| 6 | `../test-worksheets/runs/2026-09-23-zero3w-display-link-retest.md` | 视频链复测通过、`GS-HW-002` 关闭的四项判据 |
