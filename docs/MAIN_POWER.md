...
1. 使用 USB 功耗仪/电源分析仪测量候选 Linux 板：开机、屏灭、屏亮不同亮度、Wi-Fi idle、一次云端回合和 BLE 扫描/连接。
2. 用可编程电源模拟 1S 电池与 DC/DC，验证最低电压、峰值、电压跌落和正常关机。
3. 按实测平均功耗分别计算 4 小时与 8 小时电池包，制作等质量/等厚度假体。
4. 把电池假体装入三轴屏幕舱，测重心、所需转轴扭矩和导轨夹具负载。
5. 选择带保护/NTC 的实际电芯与 power-path 方案，完成充放电、边充边用和热测试。
6. 最后冻结屏幕舱厚度；不得反过来先定“很薄”再牺牲电芯安全余量。
