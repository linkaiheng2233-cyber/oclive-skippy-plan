...
| 节点 | 传感器 | 输出观察 | 状态 |
|------|--------|----------|------|
| Rail Core | 6 轴 IMU | `motion.raised/lowered/idle`、试验性 `impact.estimated` | 必做 |
| Rail Core | 霍尔 | `dock.entered/exited` | 必做 |
...
| 设备状态 | 主要证据 | 回退/否决条件 |
|----------|----------|---------------|
| Standby | `dock.entered`，或 grip released + motion idle 超时 | 节点断连时显示传感器未知，不假装仍被握持 |
| Held | `grip.engaged`，或 dock exited + 明显运动的临时推断 | grip released 且持续静止后退出 |
...
握持节点负责「手是否在握」，主模块 IMU 负责「载体是否被举起」，霍尔负责「是否在固定放置点」。三者职责独立，状态机才负责组合。
...
|------|--------------|------------|--------------|
| Grip Node 电子盒（不含压力膜） | ≤35 × 25 × 10 mm | ≤25 × 18 × 7 mm | 电池、天线净空、固定方式 |
...
- TI DRV5032 霍尔器件的 X2SON 封装约 1.4 × 1.1 mm。

最终节点通常由电池、天线 keep-out、受力结构和外壳主导，而不是传感器芯片。原型优先使用带天线的 BLE 模块；只有尺寸实测确实不够，才进入裸 SoC 自定义射频 PCB。

## 7. MCU 与功耗方向

握持节点优先评估 nRF52832 级 BLE MCU：其 ADC、GPIO、I2C/SPI 和内存足以完成压力采样、滤波、校准和通知。nRF52840 可作为开发便利或需要更多固件空间时的备选，不因参数更高默认进入量产节点。

Nordic 官方规格给出的 nRF52832 System OFF 电流可低至亚微安级，而 BLE 收发是毫安级，因此节点必须事件驱动、尽量睡眠，不持续流式上传压力曲线。实际续航仍取决于模块外围、电池、连接间隔、采样策略和温度，必须以整节点实测为准。
...
- [TI DRV5032 数据表](https://www.ti.com/lit/ds/symlink/drv5032.pdf)
