...
| GS-CI-001 | P1 | OPEN · needs repository decision | Host 通过 `../oclivenewnew` 路径依赖 OCLive；独立远端 checkout 无法复现，直接加普通 CI 会失败 | 远端仓建立时，在 git pin/submodule、发布 crate、同工作流双 checkout 或受控 vendor 中选一条并记录 ADR；随后绑定冻结 SHA 的 CI |
...
| GS-REMOTE-001 | P2 | OPEN · deferred | 本地仓尚无远端，因此没有受保护分支、远端 CI 或发布 provenance | 建立远端与依赖获取策略后配置 required gate；此前不声称“CI 已绿” |
