...
本仓库是 **OCLive 四季宝器灵（Skippy Spirit）** 的独立实体宿主工程。目标是把 OCLive 角色接入语义设备事件、小屏、语音和触觉输出；当前阶段是 S0/S1，先完成契约与桌面模拟器，再接真实硬件。
...
- 即时视觉/触觉反馈不等待 LLM。LLM 只生成低频角色表达，失败时不得阻塞设备状态机。
- 射击模式默认静默；语音只能由明确的场景/物理开关策略放行。
- v0.1 不包含相机、AI 视觉、心率、精确弹药计数、Live2D、多 BLE 节点和板载任意文本 TTS。
