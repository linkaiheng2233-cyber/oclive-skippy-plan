...
**决定二：瞄准通道。** 采用**主机摄像头 + 枪上刚性标记**（PC 侧识别标记阵列 → PnP → 屏幕坐标）：绝对参考、**无漂移**、算力全在主机、与 ADR-063「PC 前置」一致。阶梯为：**无瞄准的节奏玩法（0 元）→ 打印 ArUco 标贴（0 元）→ 四灯刚性阵列（20–40 元）**。IMU 相对指向只作**无相机时的降级方案**（方位角受磁干扰与积分漂移影响，体验明显更差）。

**决定三：唯一允许的几何规则。** **「瞄准射线 × 屏幕平面」求交是允许的；任何引入弹道、初速、飞行时间、下坠、风偏、命中概率或自动修正的计算都是禁止的。** 界面称**准星 / 光点**，不称「弹着点」；**禁止任何形式的辅助瞄准、吸附、命中修正、后坐补偿**——可以**测量**玩家瞄向哪里，**不可以修正**它。

**三项配套约束**：

1. **延迟归属**：瞄准闭环在**主机视觉内闭合**（约 50–60 ms motion-to-photon @30 fps，`ESTIMATED`），**不经过枪 → PC 链路**；该链路只承载离散事件（扣扳机、换弹、举枪）。因此 ADR-063 的「事件到画面 p95 ≤ 100 ms」门**不适用于瞄准**。推论：游戏模式恰好是有线场景，USB-CDC 的毫秒级扣扳机延迟正是游戏所需。
...
2. **相机通道定位**：P1 相机原为「娱乐观察」，现需裁决它与「瞄准通道」是同一条还是两条，以及 capability 命名。
