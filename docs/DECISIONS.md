...
发射器内部空间紧张，基础款不要求拆壳或内部走线。v0.1 使用主模块内置 IMU、霍尔和物理按钮，以及一个 BLE 自供电 Grip Node。外部有线仅用于台架采样，载体内部火控信号只可能成为未来专用适配器，不进入通用套件契约。

## ADR-012：外部传感器采用 BLE 功能节点

状态：Accepted（2026-08-24）

远端传感器按功能组成节点，而不是每颗传感器一个节点。v0.1 只实现一个压力式 Grip Node，Shoulder Node 在握持节点验收后再评审。节点完成采样、去抖、迟滞与校准，只发送低频 `SensorObservation`；导轨主机融合握持、运动和在架证据后才生成 `DeviceEvent`。节点断连使相关事实进入未知态，不能沿用最后一次握持状态。
