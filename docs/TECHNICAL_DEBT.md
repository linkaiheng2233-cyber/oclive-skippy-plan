...
| GS-CONTRACT-004 | **P1** | OPEN · blocks 一切契约扩展（角速率、只读换弹、AssetRef）；2026-09-23 静态核验 | **契约是闭集枚举 + `deny_unknown_fields`，且没有版本/能力协商**：`SensorObservationKind`（`contracts/src/lib.rs:179`）为闭集，`SensorObservation` 等结构体 `deny_unknown_fields` → **任何新观测种类都无法增量加入**，只能整版本升级，主机与固件被迫同版本更新。这与 ESP32 形态「换主机不改枪、体验上限由主机性能决定」的卖点直接冲突，也让**陀螺瞄准**（`ADR-066` 首选瞄准通道，需 100 Hz 角速率流）无法落地 | 在一次 **v0.3** 中引入**能力协商**（hello + 能力集）作为载体，并把 **① 角速率观测、② `AssetRef` ID 空间、③ 只读换弹** 作为首批能力一起做；保留旧版本拒绝语义与 golden vectors。关闭条件：新契约版本 + 能力协商 + 三类新能力的 schema / fixtures / 回放测试，并跑完整 `./scripts/verify.ps1`。范围见 `PC_SIMULATION_BACKENDS.md` §9 第 2 项与 `ESP32_GUN_ASSISTANT.md` §9.3 第 4 项 |
