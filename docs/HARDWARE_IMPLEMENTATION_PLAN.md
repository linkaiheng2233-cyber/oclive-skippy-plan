...
状态：Current SSOT（**2026-09-23 更新：新增 §0 P0-A 枪侧电子主线**）  
...
> **2026-09-23 形态说明**：ADR-063 升为 `ACCEPTED`，**P0-A「PC 宿主 + 枪侧瘦客户端」成为 P0 主线**，下文 §1–§9 的**双区域节点 + 三轴主机舱描述属 P0-B（保留后置，实测门不取消）**。P0-A 的枪侧电子见下面的 **§0**，其完整设计见 `ESP32_GUN_ASSISTANT.md`。

## 0. P0-A 枪侧电子（当前主线）

**拓扑**：枪上**一块 ESP32-S3（N16R8，16 MB Flash / 8 MB PSRAM）+ SPI 小屏 + 外挂 IMU + 外部只读扳机霍尔**，小屏按既有设计固定在导轨；PC 为唯一 Host。

| 件 | 规格 | 接口 / 引脚约束 | 决策 |
|---|---|---|---|
| 主控 | ESP32-S3 **N16R8** | 内置 2.4 GHz Wi-Fi（**仅 2.4 GHz，无 5 GHz**） | D1/D2 |
| IMU | **ICM-42688-P（SPI）**，QMI8658 为回退 | **SPI + DRDY 中断**驱动采样，不用主循环轮询 | D6 |
| 扳机检测 | 数字霍尔（A3144 类）+ 3×2 mm 钕磁铁 | 任意空闲 GPIO，**不需要 ADC**；改线性霍尔（SS49E）时**必须用 ADC1（GPIO32–39）**，因 ESP32 开 Wi-Fi 时 ADC2 不可用 | D5 |
| 握持 | FSR（前/后握） | **必须接 ADC1（GPIO32–39）**；需过采样 + 曲线校准，必要时外挂 ADS1115（先实测） | 沿用既有 |
| 屏 | SPI 小屏（ST7789 类，尺寸按导轨支架定） | SPI；素材预置在 16 MB Flash，**下行只传语义不传像素** | D3 |
| 语音 | 预留 I2S 焊盘，先不装 | — | D4 |

**能源链（P0-A 只有两个电源域）**：开发期 = 电脑 USB 供电（同时烧录与调试）；正式 = 枪侧受保护可充 1S LiPo（500–1000 mAh，多数 ESP32 板无充电，需 TP4056 或选带充电型号）。**与 PC 不共地、不互供电。**

**走线原则**：不进入机匣/波箱内部；握把 FSR 与扳机霍尔走**藏在自制握把套/护木/节点外壳内的可维护短线**；不依赖磁性放置底座。

**手柄与角色分工（防混淆）**：**固件**负责采样、滤波、去零偏、定频发送（约 100 Hz 角速率）+ 时间戳；**主机**负责积分、灵敏度、平滑、回中、夹持与渲染。

**本形态的关键待实测项**（未实测前不得写成能力）：IMU 静止噪声 / 零偏重复性 / 温漂 / 预热时间 / 100 Hz 节拍抖动；FSR ADC 线性与噪声；Wi-Fi 丢包与 p95（瞄准流的**抖动**是关键指标）；枪侧 1S 续航与充电温升。

**本形态明确不需要**：Orange Pi Zero 3W、HDMI/DP 转接、主机电池、三轴机构、BLE 节点、喇叭（D4）、相机（按需才上）。
