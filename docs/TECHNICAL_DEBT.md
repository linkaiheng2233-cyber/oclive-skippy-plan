...
**2026-09-23 更新（`GS-P0-BL-2026-09-23` 基线门禁）**：已在**干净工作树、绑定提交 `ca7bdf7`** 上重跑 `./scripts/verify.ps1`，全流程 **PASS**（3 包 fmt、workspace/依赖边界 3 members、严格 Clippy、all-targets 测试、doctest、Schema 零漂移、`cargo audit` 含 1 条允许的 `chacha20 0.10.1` yanked 警告）。本周期改动全部为 `AGENTS.md` 与 `docs/`——**代码与依赖零变更**，故上一轮"30 项测试 / 298 依赖"的量化结论继续沿用，未在本文重复计数。
...
| GS-ARCH-001 | **P1** | OPEN · **前置已解除，可开工**（`ADR-063`/`ADR-065` 均于 2026-09-23 升为 ACCEPTED）；代码未动 | **感知内核在结构上无法表达「一块板承担全部感知」**（ADR-063 / `ESP32_GUN_ASSISTANT.md` 的单 ESP32 形态会直接撞上）：① `PerceptionProfile::try_new`（`reducer/profile.rs:113`）**强制两个不同** `NodeUid`，相同即 `ProfileError::DuplicateNodeBinding`；② `NodeRole` 是 `reducer.rs:51` 的**私有两值枚举**，`node_role()` 按 UID 硬分派；③ `(role, observation)` 绑定表是**白名单**（`reducer.rs:495–522`），越界组合返回 `CapabilityNotBound`——因此一块节点**不能同时**上报前握接触 + 后握接触 + 姿态估计 + 扳机触点。附带缺口：**表达侧（下行）类型一个都还没有**——`AssetRef`、表达指令、模式（自由/预设）、能力协商/hello 均只在文档（`64A-R1`）而未进 `contracts` Rust 真源 | 把「两个固定角色」泛化为**「节点 → 能力集映射」**（**已于 `ADR-065` 冻结方向与三条硬约束**），reducer 按能力分派并同时落地能力协商（推荐）；不采用「单节点冒充两个 `node_uid`」（伪造拓扑、破坏诊断）。**不需要硬件**，可在桌面用模拟器完成（约 1–2 天，`ESTIMATED`）；关闭条件：新 ADR + `contracts` 增补能力协商与表达侧类型 + reducer 泛化 + 回放/降级测试与 fixtures 同步，并跑完整 `./scripts/verify.ps1` |
| GS-CONTRACT-004 | **P1** | OPEN · blocks 一切契约扩展（角速率、只读换弹、AssetRef）；2026-09-23 核验 | **契约是闭集枚举 + `deny_unknown_fields`；版本/能力协商「设计已冻结、代码未实现」**：`SensorObservationKind`（`contracts/src/lib.rs:179`）为闭集，`SensorObservation` 等结构体 `deny_unknown_fields` → **任何新观测种类都无法增量加入**，只能整版本升级。**注意区分**：`NodeHello` 报 `supported_contract_versions` + 各版本 capabilities、Host 取最高共同版本、无交集则 `incompatible_protocol` 且不参与融合——**这套设计早已冻结**（`HARDWARE_IMPLEMENTATION_PLAN.md` 的 `78A-R1`、`ARCHITECTURE.md`、`WIRELESS_SENSOR_NETWORK.md`、`DECISIONS.md`），**缺的是实现**（`ROADMAP.md` 待办；`GS-MAINT-001` 亦记其未加入）。此外 `Capability` 枚举（`lib.rs:234`）尚无角速率一类的新成员，且契约里**一个表达侧类型都没有** | 在一次 **v0.3** 中：**实现既有 `NodeHello` 协商设计**（载体）+ 抬版本，并把 **① 角速率观测与对应 `Capability`、② `AssetRef` ID 空间、③ 只读换弹** 作为首批能力一起做，保留旧版本拒绝语义与 golden vectors。关闭条件：NodeHello 实现 + 三类新能力的 schema / fixtures / 回放测试，并跑完整 `./scripts/verify.ps1`。范围见 `PC_SIMULATION_BACKENDS.md` §9 第 2 项与 `ESP32_GUN_ASSISTANT.md` §9.3 第 4 项 |
