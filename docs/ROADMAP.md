...
- 当前传感拓扑：**P0-A（主线）为单节点**——一块 ESP32-S3 承担前握/后握/姿态/扳机，按能力集绑定（ADR-065）；**P0-B** 为前下导轨与后握把两个区域 BLE 节点。详细五链路计划见 `HARDWARE_IMPLEMENTATION_PLAN.md`。
- **形态（2026-09-23 裁决）**：ADR-063 升为 `ACCEPTED`，**P0-A「PC 宿主 + 枪侧瘦客户端」成为 P0 主线**——枪上只留一块 ESP32-S3 + 小屏，PC 为唯一 Host，体验上限由主机性能决定；P0-B 三轴一体舱保留为后续 field 形态，实测门不取消。同时 ADR-066 立「主机侧娱乐后端（屏幕打靶 / 投影靶场）」。基线 `GS-P0-BL-2026-09-23`。
...
## P0-A：PC 宿主 + 枪侧瘦客户端（当前主线）

目标：用最少的枪侧电子把「感知 → 角色 → 反馈」跑通，并把体验上限交给 PC。设计见 `ESP32_GUN_ASSISTANT.md`（D1–D6）与 `PC_SIMULATION_BACKENDS.md`。

阶段 A / G0（桌面 USB，电脑供电，零无线变量）：

- [ ] 采购最小集（约 55–130 元）：ESP32-S3 N16R8、ICM-42688-P（SPI，QMI8658 回退）、数字霍尔 + 3×2 mm 磁铁；屏按导轨支架实际尺寸单独下单。
- [ ] 桌面 USB 阶段逐项跑通：ESP32 → SPI 屏出图；IMU 举枪/放下；霍尔扳机触点。每步只加一个变量。
- [ ] **娱乐后端 G0**：扣扳机 → 主机画面反馈（无瞄准、无相机），记录时延。
- [ ] 台架实验：IMU 静止噪声 / 零偏重复性 / 温漂 / 预热时间 / 100 Hz 节拍抖动；FSR ADC 线性与噪声；Wi-Fi 丢包与 p95。

软件前置（**不需要硬件，可与硬件并行**）：

- [ ] `GS-ARCH-001` / ADR-065：感知内核节点模型泛化为「节点 → 能力集映射」，解掉单节点形态的构造阻塞。
- [ ] `GS-CONTRACT-004` / v0.3：实现既有 `NodeHello` 版本与能力协商 + 角速率观测与 `Capability` 成员 + `AssetRef` ID 空间 + 只读换弹。

阶段 B / G1（无线与玩法）：

- [ ] 换成 Wi-Fi STA，验证断连 3–5 s 降级到预设模式、恢复稳定 2–3 s 后切回。
- [ ] 陀螺速率瞄准（ADR-066 方案 A′）：固件 100 Hz 角速率流 + 零偏自动校准 + 放下枪自动回中 + 准星边缘夹持；有线与 Wi-Fi 各出一轮抖动与手感记录。
- [ ] 预设模式「战术静默」子档：不发声、低亮度、仅图标。

阶段 C / G2–G4：

- [ ] 枪上电池、装导轨支架、整枪重量与续航回填。
- [ ] 计分 / 连击 / 关卡 + 换弹玩法 + 枪娘解说。
- [ ] （按需）相机绝对瞄准 G3；投影靶场 G4。
- [ ] （候选 P1+）离线事件摘要：本地记中性计数，回宿主只同步聚合结论、**不重放时间线**。

验收门 P0-A：见 `PROJECT_BASELINE.md` §6.1。
...
**主机侧娱乐后端已按此规则立项**（ADR-066，2026-09-23）：屏幕打靶 / 投影靶场作为 `ailive-gun-spirit-host` 内的**独立模块**，不进感知内核、不依赖 OCLive 内部对象，**不急于新增第 4 个 crate**（workspace 成员数有门禁断言）。其红线（不弹道、不辅助瞄准、对外定位娱乐而非训练）见 `PC_SIMULATION_BACKENDS.md` §7 与 `AGENTS.md`。
