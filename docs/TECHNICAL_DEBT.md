...
| GS-HW-005 | P2 | OPEN · landscape orientation not yet applied | 面板原生为 **480×800 竖屏**，项目要求横向 800×480 使用；`800x480` 虽在模式列表内但未被优先选中（EDID 首选即竖屏），不能靠改 mode 解决 | 在`/boot/armbianEnv.txt`增加`fbcon=rotate:1`（方向按实装复核）后重启，确认控制台横向可读；未来图形 UI 由 renderer 旋转 |
