...
- **桌面导出件**：桌面 `个人文档` 与到货测试包中的副本一律只是导出件，本仓是唯一权威。导出与校验统一走 `scripts/export-docs.ps1`（映射表 `scripts/export-map.tsv`，22 份）：默认执行「比对 + 补导出 + 重写 `导出说明.md` 清单块」，`-Check` 只校验漂移不写入（有漂移时退出码 1）。**改动 `docs/` 下任何被导出的文件后重跑该脚本**——不要手工复制，也不要用手工哈希去追导出件。新增导出件只改映射表，不改脚本。
