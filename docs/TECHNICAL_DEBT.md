...
| GS-HW-005 | **P1** | OPEN · panel incompatible, replacement decided | 当前 3.5 英寸 480×800 HDMI 屏（EDID 名 `HDMI480x800HH`，克隆 EDID）**在板端不出图**：板端 HPD/EDID/模式/CRTC/平面/fb0 全部正常，屏在 PC 上可正常显示，驱动以 `VIC 0 + hdmi14 vsif` 发送非 CEA 模式 → 判定为**屏侧 HDMI 兼容性**（见 ADR-064）。另：面板原生为竖屏，横向 800×480 需 `fbcon=rotate`/renderer 旋转 | ① 先验证 `USB-C DP Alt → 主动式 DP→HDMI` 路线（基线原定路线，30–80 元）；② 换"能作为通用 HDMI 接收端工作、EDID 为标准 EDID"的屏，通过四项判据 + 横向落地；③ 或改用 SPI 小屏由 MCU 直驱。运行记录见 `test-worksheets/runs/2026-09-23-zero3w-panel-no-image.md` |
